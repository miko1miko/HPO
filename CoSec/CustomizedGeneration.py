...
from transformers import (
    CodeGenForCausalLM, AutoTokenizer, LogitsProcessorList, StoppingCriteriaList, 
    GenerationConfig, PreTrainedModel, set_seed, XGLMForCausalLM, GPTBigCodeForCausalLM, 
...
class ModelWithExperts:
    ...
    def _get_logits_warper(self, generation_config: GenerationConfig) -> LogitsProcessorList:
        ...
        if generation_config.top_k is not None and generation_config.top_k != 0:
            processors.append(TopKLogitsWarper(top_k=generation_config.top_k, min_tokens_to_keep=1))
        ...
        if generation_config.eta_cutoff is not None and generation_config.eta_cutoff > 0.0:
            processors.append(EtaLogitsWarper(cutoff=generation_config.eta_cutoff, min_tokens_to_keep=1))
        if generation_config.epsilon_cutoff is not None and generation_config.epsilon_cutoff > 0.0:
    ...
    def _get_logits_processor(
        ...
        if generation_config.forced_decoder_ids is not None:
            # ForceTokensLogitsProcessor 在 transformers 4.46.0 中可能不可用，使用替代方案
            try:
    ...
    def generate_with_experts(
        ...
        if generation_config is None:
            ...
            if hasattr(self, 'generation_config') and self.generation_config._from_model_config:
        ...
        if is_greedy_gen_mode:
            ...
            logits_warper_expert = None  # 初始化为None
            ...
            if model_kwargs.get('expert', False):
                ...
                expert_generation_config.min_p = model_kwargs.pop('expert_min_p', 0.0)
    ...
    def sample_with_experts(
        ...
        if model_kwargs['expert']:
            ...
            print(f"🚀 开始专家模型生成 (阈值: {model_kwargs.get('threshold', 'N/A')})")
        else:
            print("🚀 开始标准模型生成")
        ...
        while True:
            ...
            if model_kwargs['expert']:
                ...
                # 添加调试信息：显示token选择过程
                debug_mode = model_kwargs.get('debug_mode', False)
                
                if debug_mode:
                    print(f"\n=== 第 {cur_len} 步 Token 选择过程 ===")
                    print(f"Expert模型采样token: {temp_token.cpu().numpy()}")
                    print(f"原始模型采样token: {temp_lm_token.cpu().numpy()}")
                    print(f"最终选择token: {choose_token.cpu().numpy()}")
                    
                    # 显示概率信息
                    for i in range(min(3, batch_size)):  # 只显示前3个样本的详细信息
                        expert_token = temp_token[i].item()
                        lm_token = temp_lm_token[i].item()
                        final_token = choose_token[i].item()
                        
                        expert_prob = exp_porbs[i, expert_token].item()
                        lm_prob = lm_porbs[i, lm_token].item()
                        lm_prob_for_expert_token = lm_porbs[i, expert_token].item()
                        
                        ratio = lm_prob_for_expert_token / expert_prob if expert_prob > 0 else float('inf')
                        threshold = model_kwargs['threshold']
                        ...
                        print(f"样本 {i}: Expert={expert_token}(p={expert_prob:.4f}), LM={lm_token}(p={lm_prob:.4f})")
                        print(f"  Expert token在LM中的概率: {lm_prob_for_expert_token:.4f}")
                        print(f"  概率比值 (LM/Expert): {ratio:.4f}, 阈值: {threshold:.4f}")
                        print(f"  选择: {'Expert' if ratio > threshold else 'LM'} token ({final_token})")
                    
                    print("=" * 50)
                else:
                    # 简化版本：只更新统计变量，不打印详细信息
                ...
                # 非expert模式：直接使用原始模型采样
                probs = F.softmax(next_token_scores, dim=-1)
                choose_token = torch.multinomial(probs, num_samples=1)
             # 添加调试信息：显示token选择过程
            ...
            # 确保model_kwargs中有必要的键，避免KeyError
            ...
            if model_kwargs['expert']:
                # 确保expert model的model_kwargs中也有必要的键
                if 'cache_position' not in model_kwargs_expert:
                    model_kwargs_expert['cache_position'] = torch.arange(input_ids.shape[-1], device=input_ids.device)
            ...
            try:
                ...
                print(f"Warning: Error in stopping_criteria: {e}")
                should_stop = False
                
            if unfinished_sequences.max() == 0 or should_stop:
                if not synced_gpus:
                    break
                else:
        ...
        if return_dict_in_generate:
            ...
            # 显示最终统计结果
            if model_kwargs.get('expert', False) and total_decisions > 0:
                print(f"\n{'='*60}")
                print(f"🎯 生成完成！Token选择统计:")
                print(f"   总决策次数: {total_decisions}")
                print(f"   Expert模型选择: {expert_choices} 次 ({expert_choices/total_decisions*100:.1f}%)")
                print(f"   原始模型选择: {lm_choices} 次 ({lm_choices/total_decisions*100:.1f}%)")
                print(f"   阈值设置: {model_kwargs.get('threshold', 'N/A')}")
                print(f"{'='*60}")
...
class QwenModelLM(ModelWithExperts, Qwen2ForCausalLM):
    def __init__(self, config):
        super().__init__(config)
